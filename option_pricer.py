...
class EuropeanOption:
    ...
    domestic rate = r, foreign rate = q), so FX European options are a one-line
    ...
    Extensibility
    -------------
    * American options: add a CRR binomial / PSOR PDE solver subclass.
    * Local / stochastic vol: replace ``_gbm_paths`` and keep the same API.
    * Portfolio Greeks: vectorize over a list of ``EuropeanOption`` instances.
    """
...
def build_pdf_report(
    ...
    # ---- Summary / skills ----
    story.append(Paragraph("5. Summary &amp; Extensions", styles["SectionHead"]))
    story.append(
        Paragraph(
            ...
            f"round-trips to the input σ ({iv_call:.4%}), confirming solver integrity. "
            f"Parity residual is numerically zero ({parity['residual']:.2e}).",
            styles["BodyJust"],
        )
    )
    story.append(
        Paragraph(
            "<b>Natural extensions:</b> American options via CRR binomial or finite-difference "
            "PDE; local/stochastic volatility; FX Garman–Kohlhagen (set q = foreign rate); "
            "barrier / Asian path-dependent MC; SABR smile calibration; portfolio-level Greeks "
            "and scenario P&amp;L.",
            styles["BodyJust"],
        )
    )
    ...
    story.append(
        Paragraph(
            "— End of Report —  |  Generated by option_pricer.py  |  BSM Quant Toolkit",
            styles["Small"],
        )
    )
